...
public class Trooper
{
    ...
    public Trooper(int trainingTime, int trainingCost, Weapon weapon) throws InvalidTrooperDataException {
        if(TrooperDataValidator.isInvalidTime(trainingTime)) throw new InvalidTrainingTimeException("Invalid training time "+ trainingTime);
        if(TrooperDataValidator.isInvalidCost(trainingCost)) throw new InvalidTrainingCostException("Invalid training cost "+ trainingCost);
        if(TrooperDataValidator.isInvalidWeapon(weapon)) throw new InvalidWeaponException("Invalid weapon name "+ weapon);
